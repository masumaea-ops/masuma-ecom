    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Sales Today', value: stats?.todaysOrders || 0, icon: ShoppingCart, color: 'bg-blue-500', change: 'Count', link: 'sales_history' },
          { label: 'Low Stock', value: stats?.lowStockItems || 0, icon: AlertTriangle, color: 'bg-red-500', change: 'Action Req', link: 'inventory' },
          { label: 'Pending Quotes', value: stats?.pendingQuotes || 0, icon: Users, color: 'bg-gray-600', change: 'General', link: 'quotes' },
          { label: 'Sourcing Queue', value: sourcingCount, icon: Plane, color: 'bg-purple-600', change: 'High Priority', link: 'quotes' },
        ].map((stat, i) => (
          <div 
            key={i} 
            onClick={() => onNavigate(stat.link)}
            className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 transition hover:shadow-md cursor-pointer hover:border-masuma-orange hover:scale-[1.02]"
          >
            <div className="flex justify-between items-start mb-3">
              <div className={`p-2 rounded-lg text-white shadow-sm ${stat.color}`}>
                <stat.icon size={20} />
              </div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${stat.change === 'High Priority' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'}`}>
                {stat.change}
              </span>
            </div>
            <div>
