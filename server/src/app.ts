...
app.use('/media', express.static(path.join(process.cwd(), 'media')) as any);
...
// --- STATIC FRONTEND SERVING (PRODUCTION READY) ---
// We look for the 'dist' folder in several common locations
const possibleDistPaths = [
    path.join(process.cwd(), 'dist'),                         // Root of project
    path.join(process.cwd(), '..', 'dist'),                   // Parent (standard server setup)
    '/home/kemasuma/htdocs/masuma.africa/dist',              // Absolute production path
];
...
if (!frontendPath) {
    console.warn('⚠️ [SERVER] WARNING: dist folder not found. Website will not load.');
}
...
// Fix: Use exchangeRoutes instead of exchangeRates
...
// Catch-all route to serve index.html (Vital for Password Reset links and SPA routing)
app.get('*', (req: any, res: any) => {
    // API 404s stay as JSON
    if (req.path.startsWith('/api/')) return res.status(404).json({ error: 'Endpoint not found' });
    
    // Serve index.html if frontendPath was found
    if (frontendPath) {
    ...
    // Explicit error page if dist is missing
    res.status(404).send(`
        <html>
            ...
            <body style="font-family: sans-serif; text-align: center; padding: 50px; background: #f9f9f9;">
                <div style="max-width: 500px; margin: auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-top: 5px solid #E0621B;">
                    ...
                    <div style="text-align: left; background: #eee; padding: 15px; border-radius: 5px; font-family: monospace; font-size: 13px;">
                        # RECOVERY STEPS:<br>
                        1. Connect via SSH<br>
                        2. Run: <b>cd /home/kemasuma/htdocs/masuma.africa</b><br>
                        3. Run: <b>npm run build</b><br>
                        4. Run: <b>pm2 restart kemasuma</b>
                    </div>
                </div>
            </body>
        </html>
